...
func TestCSVWriter(t *testing.T) {
	...
	t.Run("export จำนวนเงินที่ต้องโอนสำหรับ individual income", func(t *testing.T) {
		...
		assert.Equal(t, "95.00", csvColumns[NET_DAILY_INCOME_INDEX])
		assert.Equal(t, "950.00", csvColumns[NET_SPECIAL_INCOME_INDEX])
		assert.Equal(t, "0.00", csvColumns[LOAN_DEDUCTION_INDEX])
		assert.Equal(t, "", csvColumns[VAT_INDEX])
		assert.Equal(t, "55.00", csvColumns[WITHHOLDING_TAX_INDEX])
		assert.Equal(t, "1,045.00", csvColumns[TRANSFER_AMOUNT_INDEX])
